...
stage_price int not null comment '연극 티켓 가격',
...
mem_idx int not null comment '회원 고유번호',
stageinfo_idx int not null comment '공연정보 고유번호',
book_totalprice int not null comment '총 결제금액',
book_success varchar(10) not null comment '결제(성공)여부',
foreign key(mem_idx) references tcc_member(mem_idx),
foreign key(stageinfo_idx) references tcc_stageinfo(stageinfo_idx)
)engine=InnoDB default charset=utf8;
...
insert into tcc_admin (admin_id, admin_pw, admin_tel, admin_name, admin_grade, admin_email) values ('admin','ticatcher0309','0212345678','총관리자','super','admin@ticatcher.com');
